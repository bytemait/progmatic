...
const Contest: React.FC<ContestTypes> = () => {
  ...
  const user = useAppSelector((state: RootState) => state.user.details?.login);
  ...
  const fetchMycontests = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_HOST}/api/user/dashboard/${user}`
      );
      if (response.data.success && Array.isArray(response.data.contests)) {
    ...
    <div className="pb-12 bg-[#111010]">
      <div className=" pt-24 text-center">
        <img
          src="/trophy.jpg"
          alt="trophy"
          className=" h-56 rounded-full mx-auto shadow-sm-light "
        />
        <h1 className="mt-2 text-4xl text-light text-center">
          Progmatic Coding Contests
        </h1>
        <p className="mt-1 text-gray-400">
          Your Skills. Our Stage. Claim the Trophy!
        </p>
      </div>
      ...
      <div className=" bg-[#262629] shadow-xl mt-16 rounded-xl mx-auto w-2/3 ">
        <div className=" flex justify-center gap-6 p-4 ">
          <button
            className={`py-2 px-4 rounded-lg text-lg font-semibold ${
              isActiveButton === "button1"
                ? "bg-[#434345] text-white"
                : "text-[#c3c3c4]"
            } hover:text-light`}
        ...
        <div>
          <div
            className={`flex flex-wrap gap-6 p-6 justify-center  ${
              isActiveButton === "button1" ? "visible" : "hidden"
            }`}
          ></div>
          <div
            className={`flex flex-wrap gap-6 p-6 justify-center  ${
              isActiveButton === "button2" ? "visible" : "hidden"
            }`}
          >
            {contests.map(
              (
                contest: ContestTypes & { contestName: string; _id: string }
              ) => (
                <>
                  <ContestCard
                    key={contest._id}
                    contest={contest}
                    openModal={() => openModal(contest)}
                  />
                </>
              )
            )}
          ...
          <div
            className={`flex flex-wrap gap-6 p-6 justify-center  ${
              isActiveButton === "button3" ? "visible" : "hidden"
            }`}
          >
            {Array.isArray(myContests) && myContests.length > 0 ? (
              myContests.map(
                (
                  contest: ContestTypes & { contestName: string; _id: string }
                ) => (
              ...
              <p>No contests available</p> // Message when no contests are found
            )}
          </div>
        </div>
      </div>
      ...
      {isModalOpen && selectedContest && (
        <ContestInfoModal contest={selectedContest} closeModal={closeModal} />
      )}
    </div>
  );
};
